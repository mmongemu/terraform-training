...
output "my_new_cp_code" {
    value = data.akamai_cp_code.my_cp_code.id
}
...
data "akamai_property_rules_builder" "default_rule" {
    ...
    behavior {
      cp_code {
        value {
          id   = data.akamai_cp_code.my_cp_code.id
          name = data.akamai_cp_code.my_cp_code.name
        }
      }
    }
    children = [
...
output "default_rule" {
  value = data.akamai_property_rules_builder.default_rule
} 

resource "akamai_property" "my_s4_property" {
